...
class GameAnalyticsService:
    ...
    def get_comprehensive_analytics(
        ...
        if outcome_filter:
            if team_id:
                ...
                if outcome_filter == 'W':
                    ...
                    game_filters &= (
                        Q(home_team_id=team_id, home_team_score__gt=F('away_team_score')) |
                        Q(away_team_id=team_id, away_team_score__gt=F('home_team_score'))
                    )
                elif outcome_filter == 'L':
                    # Get games where team lost
                    game_filters &= (
                        Q(home_team_id=team_id, home_team_score__lt=F('away_team_score')) |
                        Q(away_team_id=team_id, away_team_score__lt=F('home_team_score'))
                    )
            else:
        ...
        if game_filters:
            filtered_game_ids = list(Game.objects.filter(game_filters).values_list('id', flat=True))
        ...
        if home_away_filter:
            if team_id:
                ...
                if home_away_filter == 'Home':
                    ...
                    possessions = possessions.filter(
                        team_id=F('game__home_team_id')
                    )
                elif home_away_filter == 'Away':
                    # All away team possessions
                    possessions = possessions.filter(
                        team_id=F('game__away_team_id')
                    )
        ...
        offensive_possessions = possessions.filter(offensive_sequence__isnull=False).exclude(offensive_sequence='')
        defensive_possessions = possessions.filter(defensive_sequence__isnull=False).exclude(defensive_sequence='')
    ...
    @staticmethod
    def _calculate_summary_stats(possessions, offensive_possessions, defensive_possessions, team_id):
        ...
        offensive_points = sum(p.points_scored for p in offensive_possessions if p.points_scored)
        defensive_points = sum(p.points_scored for p in defensive_possessions if p.points_scored)
        ...
        # Time analysis
        avg_possession_time = possessions.aggregate(
        ...
        return {
            ...
            'total_ppp': round(total_points / total_possessions, 2) if total_possessions > 0 else 0
        }
    ...
    def _analyze_offensive_possessions(offensive_possessions, team_id):
        ...
        paint_touch_analysis = GameAnalyticsService._analyze_paint_touches(offensive_possessions)
        
        # Kick Out Analysis
        kick_out_analysis = GameAnalyticsService._analyze_kick_outs(offensive_possessions)
        
        # Extra Pass Analysis
        extra_pass_analysis = GameAnalyticsService._analyze_extra_passes(offensive_possessions)
        
        # Offensive Rebound Analysis
        off_reb_analysis = GameAnalyticsService._analyze_offensive_rebounds(offensive_possessions)
        
        # Shot Time Analysis
        shot_time_analysis = GameAnalyticsService._analyze_shot_times(offensive_possessions)
        
        # Shot Quality Analysis
        shot_quality_analysis = GameAnalyticsService._analyze_shot_quality(offensive_possessions)
        
        # After Timeout Analysis
        ato_analysis = GameAnalyticsService._analyze_after_timeout(offensive_possessions)
    ...
    def _analyze_defensive_possessions(defensive_possessions, team_id):
        ...
        def_reb_analysis = GameAnalyticsService._analyze_defensive_rebounds(defensive_possessions)
    ...
    def _analyze_paint_touches(possessions):
        ...
        paint_touch_points = sum(p.points_scored for p in paint_touch_possessions if p.points_scored)
        paint_touch_ppp = paint_touch_points / total_paint_touches if total_paint_touches > 0 else 0
    ...
    def _analyze_kick_outs(possessions):
        ...
        kick_out_points = sum(p.points_scored for p in kick_out_possessions if p.points_scored)
    ...
    def _analyze_extra_passes(possessions):
        ...
        extra_pass_points = sum(p.points_scored for p in extra_pass_possessions if p.points_scored)
        extra_pass_ppp = extra_pass_points / total_extra_passes if total_extra_passes > 0 else 0
    ...
    def _analyze_offensive_rebounds(possessions):
        ...
        off_reb_points = sum(p.points_scored for p in off_reb_possessions if p.points_scored)
    ...
    def _analyze_shot_times(possessions):
        ...
        shot_time_ranges = {
            ...
            '6-10s': possessions.filter(duration_seconds__gt=5, duration_seconds__lte=10),
            '11-15s': possessions.filter(duration_seconds__gt=10, duration_seconds__lte=15),
            '16-20s': possessions.filter(duration_seconds__gt=15, duration_seconds__lte=20),
        ...
        for range_name, range_possessions in shot_time_ranges.items():
            if range_possessions.exists():
                ...
                points = sum(p.points_scored for p in range_possessions if p.points_scored)
    ...
    def _analyze_pnr_defense(defensive_possessions):
        ...
        pnr_defense_points_allowed = sum(p.points_scored for p in pnr_defense if p.points_scored)
        pnr_defense_ppp_allowed = pnr_defense_points_allowed / total_pnr_defense if total_pnr_defense > 0 else 0
    ...
    def _analyze_box_outs(defensive_possessions):
        ...
        box_out_points_allowed = sum(p.points_scored for p in box_out_possessions if p.points_scored)
        box_out_ppp_allowed = box_out_points_allowed / total_box_outs if total_box_outs > 0 else 0
    ...
    def _analyze_defensive_rebounds(defensive_possessions):
        ...
        return {
            'total_defensive_rebounds': total_def_rebs
        }
    ...
    def _analyze_player_performance(possessions, team_id, min_possessions):
        ...
        for possession in possessions:
            ...
            for player in possession.players_on_court.all():
                ...
                if possession.offensive_sequence and possession.offensive_sequence.strip():
                    player_stats[player.id]['offensive_possessions'] += 1
                if possession.defensive_sequence and possession.defensive_sequence.strip():
        ...
        for player_id, stats in player_stats.items():
            if stats['possessions'] >= min_possessions:
                stats['ppp'] = round(stats['points'] / stats['possessions'], 2) if stats['possessions'] > 0 else 0
                stats['offensive_ppp'] = round(stats['points'] / stats['offensive_possessions'], 2) if stats['offensive_possessions'] > 0 else 0
                qualified_players[player_id] = stats
        
        # Sort by PPP
        sorted_players = sorted(qualified_players.items(), key=lambda x: x[1]['ppp'], reverse=True)
    ...
    def _get_detailed_breakdown(possessions, team_id):
        ...
        for quarter in range(1, 6):  # 1-4 quarters + OT
            ...
            if quarter_possessions.exists():
                ...
                points = sum(p.points_scored for p in quarter_possessions if p.points_scored)
                ppp = points / count if count > 0 else 0
        ...
        for location in home_away_breakdown:
            ...
            home_away_breakdown[location]['ppp'] = round(points / count, 2) if count > 0 else 0
    ...
    def get_post_game_report(game_id, team_id):
        ...
        try:
            ...
            return {
                ...
                'summary': GameAnalyticsService._calculate_summary_stats_legacy(game, team_possessions, opponent_possessions),
            }
