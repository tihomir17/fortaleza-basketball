...
class GamePermissionsTests(APITestCase):
    def setUp(self):
        ...
        self.comp = Competition.objects.create(name="League", season="2024-2025", created_by=self.coach)
        self.team_a = Team.objects.create(name="A", created_by=self.coach, competition=self.comp)
        self.team_b = Team.objects.create(name="B", created_by=self.coach, competition=self.comp)
    ...
    def test_list_scoped_to_member_teams(self):
        ...
        game = Game.objects.create(competition=self.comp, home_team=self.team_a, away_team=self.team_b, game_date=timezone.now())
    ...
    def test_coach_can_create_game(self):
        ...
        res = self.client.post(self.games_url, {
