...
class AnalyticsPDFExporter:
    ...
    def export_analytics_to_pdf(self, analytics_data: Dict[str, Any], filters_applied: Dict[str, Any]) -> bytes:
        """
        ...
        if analytics_data.get('offensive_analysis'):
            story.extend(self._create_offensive_analysis_section(analytics_data['offensive_analysis']))
            story.append(Spacer(1, 20))
        
        # Add defensive analysis
        if analytics_data.get('defensive_analysis'):
            story.extend(self._create_defensive_analysis_section(analytics_data['defensive_analysis']))
            story.append(Spacer(1, 20))
        
        # Add player analysis
        if analytics_data.get('player_analysis'):
            story.extend(self._create_player_analysis_section(analytics_data['player_analysis']))
            story.append(Spacer(1, 20))
        
        # Add detailed breakdown
        if analytics_data.get('detailed_breakdown'):
            story.extend(self._create_detailed_breakdown_section(analytics_data['detailed_breakdown']))
    ...
    def _create_report_metadata(self, filters_applied: Dict[str, Any]) -> List:
        ...
        metadata = [
            ...
            ["Total Possessions Analyzed", str(filters_applied.get('total_possessions_analyzed', 0))],
        ]
        ...
        if filters_applied.get('quarter_filter'):
            metadata.append(["Quarter Filter", f"Quarter {filters_applied['quarter_filter']}"])
        
        if filters_applied.get('last_games'):
            metadata.append(["Games Filter", f"Last {filters_applied['last_games']} games"])
        
        if filters_applied.get('outcome_filter'):
            outcome_map = {'W': 'Wins', 'L': 'Losses'}
            metadata.append(["Outcome Filter", outcome_map.get(filters_applied['outcome_filter'], filters_applied['outcome_filter'])])
    ...
    def _create_summary_section(self, summary_data: Dict[str, Any]) -> List:
        ...
        summary_table_data = [
            ...
            ["Offensive Possessions", str(summary_data.get('offensive_possessions', 0))],
            ["Defensive Possessions", str(summary_data.get('defensive_possessions', 0))],
            ...
            ["Average Possession Time", f"{summary_data.get('avg_possession_time', 0):.1f}s"],
        ]
    ...
    def _create_offensive_analysis_section(self, offensive_data: Dict[str, Any]) -> List:
    ...
    def _create_defensive_analysis_section(self, defensive_data: Dict[str, Any]) -> List:
    ...
    def _create_player_analysis_section(self, player_data: Dict[str, Any]) -> List:
        ...
        story.append(Paragraph(f"Minimum Possessions Threshold: {player_data.get('min_possessions_threshold', 0)}+", self.normal_style))
        
        if player_data.get('players'):
            story.append(Paragraph("Player Statistics", self.subsection_style))
            
            # Create player table
            player_table_data = [["Player", "Possessions", "Points", "PPP", "Offensive PPP"]]
    ...
    def _create_detailed_breakdown_section(self, breakdown_data: Dict[str, Any]) -> List:
        ...
        if breakdown_data.get('home_away_breakdown'):
            ...
            story.append(self._create_data_table(home_away_data, colors.lightgoldenrodyellow))
        
        return story
    
    def _create_data_table(self, data: List[List[str]], background_color: colors.Color) -> Table:
