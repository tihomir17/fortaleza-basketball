...
class CompetitionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("name", "season")}),
        (
            "Rules & Settings",
            {
                "fields": (
                    ...
                    "shot_clock_seconds",
                    "personal_foul_limit",
                    "team_fouls_for_bonus",
                )
            },
        ),
        ("Location & Level", {"fields": ("country", "league_level")}),
    )
        ...
        if request.method == "POST":
            ...
            if action == "add_team" and team_id:
                ...
                if team.competition != competition:
                    ...
                    team.save()
                    messages.success(
                        request, f"Team {team.name} added to {competition.name}"
                    )
                else:
                    messages.warning(
                        request, f"Team {team.name} is already in {competition.name}"
                    )
                ...
                team.save()
                messages.success(
                    request, f"Team {team.name} removed from {competition.name}"
                )
    ...
    def schedule_game_view(self, request, competition_id):
            ...
            try:
                with transaction.atomic():
                    ...
                    if not all([home_team_id, away_team_id, game_date, game_time]):
                        ...
                        return redirect(
                            "admin:competition_schedule_game",
                    ...
                    if home_team == away_team:
                        messages.error(
                            request, "Home and away teams cannot be the same"
                        )
                        return redirect(
                            "admin:competition_schedule_game",
                            competition_id=competition_id,
                    ...
                    messages.success(
                        request,
                        f'Game scheduled: {home_team.name} vs {away_team.name} on {game_datetime.strftime("%Y-%m-%d %H:%M")}',
                    )
                    return redirect(
                        "admin:competitions_competition_change", competition_id
                    )
        ...
        return render(
            request, "admin/competition_management/schedule_game.html", context
        )
    ...
    if request.POST.get("post"):
        ...
        if competition_id:
            ...
            modeladmin.message_user(
                request,
                f"Successfully assigned {queryset.count()} teams to {competition.name}",
            )
        else:
            modeladmin.message_user(
                request, "Please select a competition", level="ERROR"
            )
