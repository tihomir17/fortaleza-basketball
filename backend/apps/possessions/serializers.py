...
class PossessionSerializer(serializers.ModelSerializer):
    ...
    opponent_id = serializers.PrimaryKeyRelatedField(queryset=Team.objects.all(), source='opponent', write_only=True, required=False, allow_null=True)

    class Meta:
        model = Possession
        fields = [
            'id', 'game', 'team', 'opponent', 'start_time_in_game',
            'duration_seconds', 'quarter', 'outcome', 'offensive_sequence',
            'defensive_sequence', 'logged_by', 'game_id', 'team_id', 'opponent_id',
        ]
